...
if (isset($_POST['ClubMonth'])) {
  ...
  $fetch = mysqli_query($conn, "SELECT * FROM club WHERE DATE_FORMAT(date_added, '%Y-%m') BETWEEN '$startmonth' AND '$endmonth' ORDER BY clubName");
  if (mysqli_num_rows($fetch) > 0) {

    $club = [
      ['No.', 'Club Name', 'Club Status', 'Date added',]
    ];
    ...
    foreach ($fetch as $row) {
      ...
      if ($row['clubStatus'] == 0) {
        $status = 'Pending';
      } elseif ($row['clubStatus'] == 1) {
        $status = 'Approved';
      } else {
        $status = 'Denied';
      }
      $club = array_merge($club, array(array($id, $row['clubName'], $status, date("F d, Y", strtotime($row['date_added'])))));
    }

    $xlsx = SimpleXLSXGen::fromArray($club);
    $xlsx->downloadAs('Club monthly report lists.xlsx'); // This will download the file to your local system
    // $xlsx->saveAs('resident.xlsx'); // This will save the file to your server
    echo "<pre>";
    print_r($club);
...
if (isset($_POST['ClubYear'])) {
  ...
  $fetch = mysqli_query($conn, "SELECT * FROM club WHERE YEAR(date_added) BETWEEN '$startyear' AND '$endyear' ORDER BY clubName");
  if (mysqli_num_rows($fetch) > 0) {
    ...
    $xlsx = SimpleXLSXGen::fromArray($club);
    $xlsx->downloadAs('Club yearly report lists.xlsx'); // This will download the file to your local system
    // $xlsx->saveAs('resident.xlsx'); // This will save the file to your server
    echo "<pre>";
    print_r($club);
    header('Location: reports.php');
...
if (isset($_POST['attendanceMonth'])) {
  $startmonth = $_POST['startmonth'];
  $endmonth = $_POST['endmonth'];
  $fetch = mysqli_query($conn, "SELECT * FROM attendance JOIN requestletter ON attendance.eventName=requestletter.requestId JOIN users ON attendance.user_Id=users.user_Id WHERE users.user_type='Member' AND users.account_status=1 AND DATE_FORMAT(attendance.date_added, '%Y-%m') BETWEEN '$startmonth' AND '$endmonth' ORDER BY attendanceId DESC");
  if (mysqli_num_rows($fetch) > 0) {
...
if (isset($_POST['attendanceYear'])) {
  $startyear = $_POST['startyear'];
  $endyear = $_POST['endyear'];
  $fetch = mysqli_query($conn, "SELECT * FROM attendance JOIN requestletter ON attendance.eventName=requestletter.requestId JOIN users ON attendance.user_Id=users.user_Id WHERE users.user_type='Member' AND users.account_status=1 AND YEAR(attendance.date_added) BETWEEN '$startyear' AND '$endyear' ORDER BY attendanceId DESC");
  if (mysqli_num_rows($fetch) > 0) {
