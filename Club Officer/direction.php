                ...
                <table id="example111" class="table table-bordered table-hover text-sm">
                  ...
                  <tbody id="users_data">
                    <?php 
                      $i = 1;
                      $club = $row['club'];
                      $sql = mysqli_query($conn, "SELECT * FROM ride_direction JOIN users ON ride_direction.added_by=users.user_Id WHERE (user_type='BROD' || club='$club') ORDER BY ride_id DESC");
                    ...
                    <tr>
                      ...
                      <td class="bg-grey text-muted">
                        <?php if($row['user_type'] == 'BROD'): ?>
                          <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#update<?php echo $row['ride_id']; ?>" disabled><i class="fas fa-pencil-alt"></i> Edit</button>
                          <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#delete<?php echo $row['ride_id']; ?>" disabled><i class="fas fa-trash"></i> Delete</button>
